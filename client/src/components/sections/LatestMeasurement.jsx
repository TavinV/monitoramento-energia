...
const descriptions = {
    ...
    energy: "Energia consumida desde a última medição (kWh).",
    cost: "Custo estimado do consumo desde a última medição (Reais).",
...
function LatestMeasurement() {
    ...
    // enquanto carrega, não tenta acessar valores inexistentes
    const m = Array.isArray(measurements) ? measurements[0] : measurements;

    // se ainda estiver carregando ou sem dados
    if (loading || !m) {
        return (
            <div className="flex flex-col bg-gray-200 p-5 rounded-xl shadow-md mb-10">
                <h2 className="sm:text-md text-sm text-center sm:text-left mb-3 font-semibold text-gray-700">
                    Carregando dados da última medição...
                </h2>
                <CardGrid>
                    {["voltage", "current", "power", "energy", "cost", "timestamp"].map((type) => (
                        <Card key={type} type={type} loading={true} />
                    ))}
                </CardGrid>
            </div>
        );
